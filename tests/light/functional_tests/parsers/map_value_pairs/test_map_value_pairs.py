...
#############################################################################
MESSAGES_PER_TAG = 99
TAGS = ("map_value_pairs1", "map_value_pairs2")
TEMPLATE = r'"${ISODATE} ${HOST} ${MSGHDR}${foo.MSG}\n"'
...
def test_map_value_pairs(config, syslog_ng, port_allocator):
    ...
    map_value_pairs = config.create_map_value_pairs_parser(key='"MSG*" rekey(add-prefix("foo."))')
    file_destination = config.create_file_destination(file_name="output.log", template=TEMPLATE)
    config.create_logpath(statements=[source, map_value_pairs, file_destination])
