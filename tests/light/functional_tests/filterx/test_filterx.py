...
def test_nullv_set_subscript_skips_the_key_on_a_null_rhs(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
set_pri(30);
l = [1, 2];
...
""",
    ...
    assert file_true.get_stats()["processed"] == 1
    assert "processed" not in file_false.get_stats()
    assert file_true.read_log() == "30"


def test_nullv_set_subscript_skips_the_key_on_a_failing_rhs(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
set_pri(30);
l = [1, 2];
...
def test_parse_csv_nonstring_argument(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
    custom_message = 567;
...
def test_fix_timezone_invalid_number_of_args(config, syslog_ng):
    _ = create_config(
        config, """
        $MSG = fix_timezone("WET");
""",
    )
    with pytest.raises(Exception) as exec_info:
        syslog_ng.start(config)
...
def test_fix_timezone_wrong_arguments(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
        datetime = strptime("2000-01-01T00:00:00 +0200", "%Y-%m-%dT%H:%M:%S %z");
        $MSG = fix_timezone(datetime, 1500);
""",
    )
    syslog_ng.start(config)
...
def test_fix_timezone_unknown_timezone(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
        datetime = strptime("2000-01-01T00:00:00 +0200", "%Y-%m-%dT%H:%M:%S %z");
        fixed_datetime = fix_timezone(datetime, "HKT");
""",
    )
    syslog_ng.start(config)
...
def test_fix_timezone_success_variable_timezone(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
        datetime = strptime("2000-01-01T00:00:00 +0200", "%Y-%m-%dT%H:%M:%S %z");
        timezone = "CET";
...
""",
    ...
    assert file_true.get_stats()["processed"] == 1
    assert "processed" not in file_false.get_stats()
    assert file_true.read_log() == "2000-01-01T00:00:00 +0100"


def test_fix_timezone_success(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
        datetime = strptime("2000-01-01T00:00:00 +0200", "%Y-%m-%dT%H:%M:%S %z");
        fixed_datetime = fix_timezone(datetime, "CET");
        $MSG = strftime("%Y-%m-%dT%H:%M:%S %z", fixed_datetime);
""",
    )
    syslog_ng.start(config)
...
def test_guess_timezone_invalid_number_of_args(config, syslog_ng):
    _ = create_config(
        config, """
        $MSG = guess_timezone();
""",
...
def test_guess_timezone_wrong_argument_type(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
        $MSG = guess_timezone("%Y-%m-%dT%H:%M:%S %z");
""",
    )
    syslog_ng.start(config)
...
def test_guess_timezone_success(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
        datetime = strptime("2000-01-01T00:00:00", "%Y-%m-%dT%H:%M:%S");
        guessed_datetime = guess_timezone(datetime);
        $MSG = strftime("%Y-%m-%dT%H:%M:%S %z", guessed_datetime);
""",
    )
    syslog_ng.start(config)
...
def test_set_timezone_wrong_arguments(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
        datetime = strptime("2000-01-01T00:00:00 +0200", "%Y-%m-%dT%H:%M:%S %z");
        $MSG = set_timezone(datetime, 1500);
""",
    )
    syslog_ng.start(config)
...
def test_set_timezone_unknown_timezone(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
        datetime = strptime("2000-01-01T00:00:00 +0200", "%Y-%m-%dT%H:%M:%S %z");
        set_datetime = set_timezone(datetime, "HKT");
""",
    )
    syslog_ng.start(config)
...
def test_set_timezone_success_variable_timezone(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
        datetime = strptime("2000-01-01T00:00:00 +0200", "%Y-%m-%dT%H:%M:%S %z");
        timezone = "CET";
...
def test_set_timezone_success(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
        datetime = strptime("2000-01-01T00:00:00 +0200", "%Y-%m-%dT%H:%M:%S %z");
...
def test_get_timezone_source_fix_fixed_success(config, syslog_ng):
    (file_true, file_false) = create_config(
        config, """
        datetime = strptime("2000-01-01T00:00:00", "%Y-%m-%dT%H:%M:%S");
        fixed_datetime = fix_timezone(datetime, "EET");
