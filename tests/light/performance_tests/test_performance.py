...
def build_destination(destination, config, config_statements, clickhouse_server, request, testcase_parameters):
    ...
    if destination == "clickhouse-dst-for-filterx-format":
        clickhouse_options = {
            "database": "default",
            "table": "test_table",
            "user": "default",
...
def test_performance(request, testcase_parameters, config, syslog_ng, clickhouse_server, port_allocator, loggen, input_msg_type, msg_size, msg_counter, filterx_rule, destination, flow_control, run_time):
    ...
    if destination:
        config_statements, destination_driver = build_destination(destination, config, config_statements, clickhouse_server, request, testcase_parameters)
