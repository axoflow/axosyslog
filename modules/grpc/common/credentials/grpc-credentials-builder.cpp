  ...
  switch (mode)
    ...
    case GCAM_ADC:
    {
      if (!adc_service_account_key.empty())
        {
          const char *key_path_orig = getenv("GOOGLE_APPLICATION_CREDENTIALS");
          if (setenv("GOOGLE_APPLICATION_CREDENTIALS", adc_service_account_key.c_str(), 1) != 0)
            {
          ...
          if (setenv("GOOGLE_APPLICATION_CREDENTIALS", key_path_orig, 1) != 0)
            {
