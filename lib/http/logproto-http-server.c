...
log_proto_http_server_poll_prepare(LogProtoServer *s, GIOCondition *cond, gint *timeout)
{
  ...
  if (self->state == STATE_SEND_HTTP_RESPONSE || self->state == STATE_HTTP_ERROR)
    ...
    {
      reschedule_now = buffer_size(&self->in_buffer) != 0;
  ...
  return reschedule_now ? LPPA_FORCE_SCHEDULE_FETCH : LPPA_POLL_IO;
}
...
static HTTPRequest *
log_proto_http_server_receive_request(LogProtoHTTPServer *self, LogProtoStatus *status)
{
  ...
  while (self->state == STATE_RECEIVE_HTTP_REQUEST)
    {
      if (buffer_is_empty(&self->in_buffer))
        {
          GIOStatus io_status = log_proto_http_server_fetch_data(self);
  ...
  for (gint requests = 0; requests < MAX_REQUESTS_PER_FETCH; )
    {
      switch (self->state)
        ...
        case STATE_RECEIVE_HTTP_REQUEST:
          ;
          HTTPRequest *http_request = log_proto_http_server_receive_request(self, &status);
          if (http_request)
            {
...
log_proto_http_server_free(LogProtoServer *s)
{
  ...
  if (self->pending_log_messages)
    g_queue_free_full(self->pending_log_messages, (GDestroyNotify) log_msg_unref);
