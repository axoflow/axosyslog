...
FilterXObject *
filterx_ref_get_readonly_value(FilterXObject *s)
{
...
FilterXObject *filterx_ref_get_writable_value(FilterXObject *s)
{
  if (!s || !filterx_object_is_type(s, &FILTERX_TYPE_NAME(ref)))
    return s;
...
{
  FilterXRef *self = (FilterXRef *) s;
  return filterx_object_map_to_json(self->value, object, assoc_object);
}
...
filterx_ref_new(FilterXObject *value)
{
  ...
  self->super.make_readonly = _prohibit_readonly;
