...
Test(filterx_type_inference, folded_literal_dict_records_a_type_per_key)
{
  ...
  FilterXExpr *read_b = filterx_getattr_new(filterx_floating_variable_expr_new("d"),
...
Test(filterx_type_inference, folded_literal_depth3_dict_chain)
{
  ...
  FilterXExpr *read_l1 = filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                                             filterx_string_new("l1", -1));
  FilterXExpr *read_l2 = filterx_getattr_new(read_l1, filterx_string_new("l2", -1));
  FilterXExpr *read_l3 = filterx_getattr_new(read_l2, filterx_string_new("l3", -1));
  FilterXExpr *block = filterx_compound_expr_new_va(TRUE, assign, read_l3, NULL);
  block = _run(block);

  /* Each node answers from its own path, so the nesting is read off the expression tree rather
   * than walked out of one node's chain. */
  cr_assert_eq(assign->static_type, FILTERX_STATIC_TYPE_DICT);
...
Test(filterx_type_inference, a_folded_literal_list_is_not_an_empty_one)
{
  /* v = {"l": [1, 2, 3]}; v.l.anything  -> UNKNOWN, not "absent because the list is closed and
   * has no recorded children".  Nothing inside a list can be recorded, so a list installed from
   * an object in hand must give up `closed` rather than claim an empty key set -- the one place
   * in this pass where dropping the all-keys bound could have made a claim stronger. */
  GList *elems = NULL;
  for (gint i = 1; i <= 3; i++)
  ...
  FilterXExpr *read_l = filterx_getattr_new(filterx_floating_variable_expr_new("v"),
                                            filterx_string_new("l", -1));
  FilterXExpr *read_elem = filterx_get_subscript_new(
...
Test(filterx_type_inference, a_list_element_has_no_type_of_its_own)
{
   ...
   *
   * A list index names nothing -- a negative one resolves against the runtime length, and unset()
   * shifts every later index down -- so there is no location for the write to have typed and no
   * location for the read to look up.  Element types are what an all-keys bound used to buy, and
   * what dropping it costs: nothing inside a list is described any more. */
...
Test(filterx_type_inference, one_sided_branch_write_survives_the_join)
{
  /* d = {}; if (c) { d.a = {}; } d.a  -> DICT.
   *
   * The branch that did not write still has d closed, which *proves* the key absent there, so
   * that side constrains nothing and the written type survives.  Sound under the non-NULL
   * contract: if the condition was false the key does not exist and the read returns C NULL,
   * which static_type says nothing about. */
  FilterXExpr *assign_d = filterx_assign_new(
                            filterx_floating_variable_expr_new("d"),
                            filterx_literal_dict_new(NULL));
  FilterXExpr *set_a = filterx_setattr_new(
...
{
  /* Build a getattr chain far deeper than FILTERX_PATH_MAX_DEPTH.  A write past the cap has no
   * address to land on, so it opens the deepest ancestor it *can* name -- it no longer retires
   * everything under the root, which is why the shallow read below still resolves. */
  enum { DEPTH = 20 };
  cr_assert_gt(DEPTH, FILTERX_PATH_MAX_DEPTH);
  ...
  filterx_compound_expr_add_ref(block, filterx_assign_new(
                                  filterx_floating_variable_expr_new("d"),
                                  filterx_literal_dict_new(NULL)));
  /* d.k0 = {}; d.k0.k1 = {}; ... — the deepest writes open their truncated ancestor instead. */
  ...
  /* ... and so does the deepest addressable level itself. */
  FilterXExpr *read_deepest = _build_attr_chain(keys, FILTERX_PATH_MAX_DEPTH);
 ...
 *                                  just replaced.  @expected_b is what it reports instead: the
 *                                  replacement's own shape when the write names its key, and
 *                                  nothing at all when it cannot.
 * @replace_a is the expression under test; ownership is taken. */
static void
_assert_replacing_a_key_retypes_its_subtree(FilterXExpr *replace_a, FilterXStaticType expected_b)
{
  ...
  FilterXExpr *set_a_b = filterx_setattr_new(
                           ...
                           filterx_string_new("b", -1),
                           filterx_literal_new(filterx_integer_new(5)));
  FilterXExpr *read_a_b = filterx_getattr_new(
                            filterx_getattr_new(filterx_floating_variable_expr_new("v"),
                                                filterx_string_new("a", -1)),
                            filterx_string_new("b", -1));
...
Test(filterx_type_inference, setattr_replacing_a_key_installs_the_new_shape)
{
  ...
  _assert_replacing_a_key_retypes_its_subtree(
    filterx_setattr_new(filterx_floating_variable_expr_new("v"),
                        filterx_string_new("a", -1),
...
{
  /* v["a"] = {"b": "hopszika"};  -> the same store as v.a = ..., spelled with a literal key, and
   * it has to leave the same state behind.  This is the pair that used to differ: a literal
   * string subscript recorded nothing and retired the whole container. */
  _assert_replacing_a_key_retypes_its_subtree(
...
Test(filterx_type_inference, a_key_written_after_a_dynamic_write_is_knowledge)
{
  /* d = {}; d[$k] = "s"; d.a = 1; d.a  -> INTEGER.
   *
   * The whole point of not summarising over the keys this pass cannot name.  The dynamic write
   * leaves d a DICT with no leaves, so the d.a written after it is the only thing that has ever
   * been said about that location -- and a lookup that hits it is knowledge, not a bound to be
   * met with whatever the dynamic write might have put there. */
...
Test(filterx_type_inference, a_dynamic_write_keeps_the_container_and_drops_its_leaves)
{
   ...
   * than replacing it, so its kind stands; where inside it landed is not knowable, so nothing
...
Test(filterx_type_inference, a_dynamic_write_one_level_down_leaves_its_siblings_alone)
{
  ...
  FilterXExpr *set_a = filterx_setattr_new(filterx_floating_variable_expr_new("d"),
                                           filterx_string_new("a", -1),
                                           filterx_literal_dict_new(NULL));
  FilterXExpr *set_a_x = filterx_setattr_new(
                           filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                                               filterx_string_new("a", -1)),
                           filterx_string_new("x", -1),
                           filterx_literal_new(filterx_integer_new(1)));
  FilterXExpr *set_sibling = filterx_setattr_new(filterx_floating_variable_expr_new("d"),
                                                 filterx_string_new("sib", -1),
                                                 filterx_literal_new(filterx_string_new("s", -1)));
  FilterXExpr *set_dynamic = filterx_set_subscript_new(
                               filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                                                   filterx_string_new("a", -1)),
  ...
  FilterXExpr *read_a_x = filterx_getattr_new(
                            filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                                                filterx_string_new("a", -1)),
                            filterx_string_new("x", -1));
  FilterXExpr *read_a = filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                                            filterx_string_new("a", -1));
  FilterXExpr *read_sibling = filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                                                  filterx_string_new("sib", -1));
...
Test(filterx_type_inference, replacing_a_key_keeps_sibling_per_key_types)
{
  ...
  FilterXExpr *set_a = filterx_setattr_new(filterx_floating_variable_expr_new("v"),
                                           filterx_string_new("a", -1),
                                           filterx_literal_new(filterx_integer_new(1)));
  FilterXExpr *set_subscription = filterx_setattr_new(filterx_floating_variable_expr_new("v"),
                                                      filterx_string_new("subscription", -1),
                                                      filterx_literal_new(filterx_string_new("s", -1)));
  FilterXExpr *set_sub = filterx_setattr_new(filterx_floating_variable_expr_new("v"),
                                             filterx_string_new("sub", -1),
                                             filterx_literal_dict_new(NULL));
  FilterXExpr *set_sub_x = filterx_setattr_new(
                             filterx_getattr_new(filterx_floating_variable_expr_new("v"),
                                                 filterx_string_new("sub", -1)),
                             filterx_string_new("x", -1),
                             filterx_literal_new(filterx_integer_new(2)));
  FilterXExpr *replace_sub = filterx_setattr_new(filterx_floating_variable_expr_new("v"),
                                                 filterx_string_new("sub", -1),
                                                 _string_valued_dict("x", "s"));
  FilterXExpr *read_a = filterx_getattr_new(filterx_floating_variable_expr_new("v"),
                                            filterx_string_new("a", -1));
  FilterXExpr *read_subscription = filterx_getattr_new(filterx_floating_variable_expr_new("v"),
                                                       filterx_string_new("subscription", -1));
  FilterXExpr *read_sub = filterx_getattr_new(filterx_floating_variable_expr_new("v"),
                                              filterx_string_new("sub", -1));
...
Test(filterx_type_inference, writing_into_a_nested_empty_dict_commits_the_deeper_level)
{
   ...
   *
   * Asserted through v.a.b, which is the whole point: there is one place a fact about that
   * location can live, so a read of it cannot report something coarser than what the write
   * recorded.  The chain-plus-per-key-map version had two answers here and had to be asserted
   * on v's own chain to avoid the coarser one. */
  FilterXExpr *assign_v = filterx_assign_new(filterx_floating_variable_expr_new("v"),
                                             filterx_literal_dict_new(NULL));
  FilterXExpr *set_a = filterx_setattr_new(filterx_floating_variable_expr_new("v"),
                                           filterx_string_new("a", -1),
                                           filterx_literal_dict_new(NULL));
  FilterXExpr *replace_a = filterx_setattr_new(filterx_floating_variable_expr_new("v"),
                                               filterx_string_new("a", -1),
                                               _string_valued_dict("b", "x"));
  FilterXExpr *read_v = filterx_floating_variable_expr_new("v");
  FilterXExpr *read_a = filterx_getattr_new(filterx_floating_variable_expr_new("v"),
                                            filterx_string_new("a", -1));
  FilterXExpr *read_a_b = filterx_getattr_new(read_a, filterx_string_new("b", -1));
...
{
  /* $a.b = 1 and $a["b"] = 1 are the same store, so they have to leave the same state behind --
   * and it has to be readable through either spelling.  The chain-and-per-key-map version
   * recorded nothing for the subscript form and retired the whole container instead. */
  FilterXExpr *assign_getattr = filterx_assign_new(filterx_floating_variable_expr_new("a"),
                                                   filterx_literal_dict_new(NULL));
  FilterXExpr *set_getattr = filterx_setattr_new(filterx_floating_variable_expr_new("a"),
                                                 filterx_string_new("b", -1),
                                                 filterx_literal_new(filterx_integer_new(1)));
  ...
  FilterXExpr *assign_subscript = filterx_assign_new(filterx_floating_variable_expr_new("c"),
                                                     filterx_literal_dict_new(NULL));
  FilterXExpr *set_subscript = filterx_set_subscript_new(
                                 filterx_floating_variable_expr_new("c"),
                                 filterx_literal_new(filterx_string_new("b", -1)),
                                 filterx_literal_new(filterx_integer_new(1)));
  FilterXExpr *read_via_getattr = filterx_getattr_new(filterx_floating_variable_expr_new("c"),
                                                      filterx_string_new("b", -1));
...
Test(filterx_type_inference, writing_a_key_twice_overwrites_rather_than_meets)
{
   ...
   * doubt at all. */
  FilterXExpr *assign_a = filterx_assign_new(filterx_floating_variable_expr_new("a"),
                                             filterx_literal_dict_new(NULL));
  FilterXExpr *set_string = filterx_setattr_new(filterx_floating_variable_expr_new("a"),
                                                filterx_string_new("b", -1),
                                                filterx_literal_new(filterx_string_new("s", -1)));
  FilterXExpr *set_integer = filterx_setattr_new(filterx_floating_variable_expr_new("a"),
                                                 filterx_string_new("b", -1),
                                                 filterx_literal_new(filterx_integer_new(1)));
  FilterXExpr *read_b = filterx_getattr_new(filterx_floating_variable_expr_new("a"),
                                            filterx_string_new("b", -1));
...
{
  /* d = {}; d.a = {}; d.a.x = 1; d[$k].b = 2; d.a.x  -> UNKNOWN.
   *
   * The setattr's object is d[$k], which addresses no location, so the path it hands the env stops
   * truncated at d.  Peeling only one level and opening d[$k] instead would open nothing at all,
   * and d.a's interior would survive a write that may well have gone through it. */
  FilterXExpr *assign_d = filterx_assign_new(filterx_floating_variable_expr_new("d"),
                                             filterx_literal_dict_new(NULL));
  FilterXExpr *set_a = filterx_setattr_new(filterx_floating_variable_expr_new("d"),
                                           filterx_string_new("a", -1),
...
{
  /* a = {}; a.x = 1; a[$k]  -> UNKNOWN, even though a is closed over a single INTEGER key.
   *
   * Answering INTEGER would mean reasoning over the whole key set rather than over one entry, and
   * that is the reasoning the env no longer does: a[$k] addresses no location, so no entry speaks
   * for it.  The named read a.x is unaffected. */
  FilterXExpr *assign_a = filterx_assign_new(filterx_floating_variable_expr_new("a"),
                                             filterx_literal_dict_new(NULL));
  FilterXExpr *set_x = filterx_setattr_new(filterx_floating_variable_expr_new("a"),
                                           filterx_string_new("x", -1),
                                           filterx_literal_new(filterx_integer_new(1)));
  FilterXExpr *read_dynamic = filterx_get_subscript_new(filterx_floating_variable_expr_new("a"),
                                                        filterx_floating_variable_expr_new("k"));
  FilterXExpr *read_x = filterx_getattr_new(filterx_floating_variable_expr_new("a"),
                                            filterx_string_new("x", -1));
...
Test(filterx_type_inference, copying_a_container_into_its_own_subtree_terminates)
{
  ...
  FilterXExpr *set_a = filterx_setattr_new(filterx_floating_variable_expr_new("d"),
                                           filterx_string_new("a", -1),
                                           filterx_literal_new(filterx_integer_new(1)));
  FilterXExpr *self_copy = filterx_setattr_new(filterx_floating_variable_expr_new("d"),
                                               filterx_string_new("sub", -1),
                                               filterx_floating_variable_expr_new("d"));
  FilterXExpr *read_sub = filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                                              filterx_string_new("sub", -1));
  FilterXExpr *read_sub_a = filterx_getattr_new(
                              filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                                                  filterx_string_new("sub", -1)),
...
Test(filterx_type_inference, hoisting_a_subtree_over_its_own_root_terminates)
{
  ...
  FilterXExpr *assign_d = filterx_assign_new(filterx_floating_variable_expr_new("d"),
                                             filterx_literal_dict_new(NULL));
  FilterXExpr *set_sub = filterx_setattr_new(filterx_floating_variable_expr_new("d"),
                                             filterx_string_new("sub", -1),
                                             filterx_literal_dict_new(NULL));
  FilterXExpr *set_sub_a = filterx_setattr_new(
                             filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                             ...
                             filterx_string_new("a", -1),
                             filterx_literal_new(filterx_string_new("s", -1)));
  FilterXExpr *hoist = filterx_assign_new(filterx_floating_variable_expr_new("d"),
                                          filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                                                              filterx_string_new("sub", -1)));
  FilterXExpr *read_a = filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                                            filterx_string_new("a", -1));
...
Test(filterx_type_inference, a_non_literal_key_leaves_the_container_empty)
{
   ...
   * builds no template and there is no key to hang the 1 on.  The container's own kind is all
  ...
  FilterXExpr *assign_d = filterx_assign_new(filterx_floating_variable_expr_new("d"),
                                             filterx_literal_dict_new(elems));
  FilterXExpr *read_named = filterx_getattr_new(filterx_floating_variable_expr_new("d"),
                                                filterx_string_new("anything", -1));
  FilterXExpr *read_d = filterx_floating_variable_expr_new("d");
...
{
  /* d = {"a": ??$x};  -- _literal_dict_store_elem() leaves the slot unset when the value is
   * null, so the key may not exist at runtime.  Recording it would assert that it does; leaving
   * no entry is what makes a read of d.a answer UNKNOWN rather than $x's type. */
...
Test(filterx_type_inference, a_nullv_element_costs_the_container_its_closed_key_set)
{
   ...
   *
   * The nullv key is recorded nowhere, but it may well exist at runtime, so d's key set is not
   * bounded by its recorded children.  Were d left closed, the branch that did not write would
   * read as proving "a" absent and the join would keep INTEGER -- for a location that holds $x
   * whenever the condition was false and $x was not null. */
  GList *elems = g_list_append(NULL, filterx_nullv_literal_element_new(
                                 filterx_literal_new(filterx_string_new("a", -1)),
                                 filterx_floating_variable_expr_new("x")));
  FilterXExpr *assign_x = filterx_assign_new(filterx_floating_variable_expr_new("x"),
...
Test(filterx_type_inference, env_entries_outlive_the_expression_tree_they_came_from)
{
   ...
   * at :125, so an env keyed by those pointers dangles the moment the tree goes.  _run() frees
   * the env and leaves the exprs alive, which is the opposite order, so this has to be spelled
   * out by hand. */
  ...
  FilterXExpr *set_key = filterx_setattr_new(filterx_floating_variable_expr_new("v"),
                                             filterx_string_new("a_borrowed_key_name", -1),
                                             filterx_literal_new(filterx_string_new("s", -1)));
  ...
  path.root = handle;
  filterx_type_path_append_step(&path, filterx_type_path_intern_key("a_borrowed_key_name"));

  cr_assert_eq(filterx_type_env_get_at_path(env, &path), FILTERX_STATIC_TYPE_STRING);
  filterx_type_env_free(env);
}
