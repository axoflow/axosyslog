...
Test(filterx_expr_arithmetic, normal_uminus_still_works)
{
  ...
  gint64 value;
  cr_assert(filterx_integer_unwrap(res, &value));
  cr_assert(eq(i64, value, -5));
  filterx_object_unref(res);
}

Test(filterx_expr_arithmetic, double_division_by_zero_is_rejected)
{
  cr_assert(zero(ptr, _eval_binop_double(filterx_operator_division_new, 1.0, 0.0)));
}

Test(filterx_expr_arithmetic, double_zero_over_zero_is_rejected)
{
  cr_assert(zero(ptr, _eval_binop_double(filterx_operator_division_new, 0.0, 0.0)));
}

Test(filterx_expr_arithmetic, double_multiplication_overflow_is_rejected)
{
  cr_assert(zero(ptr, _eval_binop_double(filterx_operator_multiplication_new, 1e308, 1e308)));
}

Test(filterx_expr_arithmetic, double_subtraction_overflow_is_rejected)
{
  cr_assert(zero(ptr, _eval_binop_double(filterx_operator_substraction_new, -1e308, 1e308)));
}

Test(filterx_expr_arithmetic, normal_double_division_still_works)
{
  FilterXObject *res = _eval_binop_double(filterx_operator_division_new, 6.0, 2.0);
  cr_assert(not(zero(ptr, res)));
  GenericNumber gn = filterx_primitive_get_value(res);
  cr_assert(epsilon_eq(dbl, gn_as_double(&gn), 3.0, 0.00001));
  filterx_object_unref(res);
}
