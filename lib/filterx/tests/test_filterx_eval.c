...
Test(filterx_eval, test_filterx_eval_context_dup_creates_independent_snapshot)
{
  ...
  /* an arena of its own, not the thread's */
  cr_assert_not_null(dup_context->private_allocator);
  cr_assert_eq(dup_context->allocator, dup_context->private_allocator);
  cr_assert_eq(dup_context->allocator->id, FILTERX_ALLOCATOR_ID_PRIVATE);
