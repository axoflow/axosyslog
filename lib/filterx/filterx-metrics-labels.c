...
_format_value_expr(FilterXExpr *expr, gchar **formatted_value)
{
  ...
  if (!obj)
    {
      success = FALSE;
      filterx_eval_push_error_static_info("Failed to format metrics label value",
  ...
  if (!success)
    {
      filterx_eval_push_error("Failed to format metrics label value: str() failed", obj);
    }
exit:
...
_init_label_name(FilterXExpr *name)
{
  if (!filterx_expr_is_literal(name))
    {
      filterx_eval_push_error_static_info("Failed to initialize metrics label name",
  ...
  if (!str)
    {
      filterx_eval_push_error_info_printf("Failed to initialize metrics label name",
...
_format_dict_elem_to_store(FilterXObject *key, FilterXObject *value, gpointer user_data)
{
  ...
  if (!name_str)
    {
      filterx_eval_push_error_info_printf("Failed to format label name",
  ...
  if (!value_str)
    {
      filterx_eval_push_error_static_info("Failed to format label value",
...
_format_dict_to_store(FilterXObject *obj, DynMetricsStore *store, StatsClusterLabel **labels, gsize *len)
{
  ...
  if (!filterx_object_is_type(typed_obj, &FILTERX_TYPE_NAME(mapping)))
    {
      filterx_eval_push_error_info_printf("Failed to format metrics labels",
...
_format_expr(FilterXExpr *expr, DynMetricsStore *store, StatsClusterLabel **labels, gsize *len)
{
  ...
  if (!obj)
    {
      filterx_eval_push_error_static_info("Failed to format metrics labels",
                                          "Failed to evaluate expression");
      return FALSE;
    }
