...
static void
_filterx_tuple_append(FilterXTuple *self, FilterXObject *el)
{
  g_ptr_array_add(self->array, el);
  if (!filterx_object_hashable(el))
  ...
  for (gsize i = 0; i < self->array->len; i++)
    {
      ...
      _filterx_tuple_append(clone, el);
    }
  ...
  for (guint64 i = 0; i < sequence_len; i++)
    {
      ...
      _filterx_tuple_append(clone, filterx_object_copy(elt));
...
filterx_tuple_new_from_sequence(FilterXObject *sequence)
{
  ...
  for (guint64 i = 0; i < len; i++)
    {
      FilterXObject *elt = filterx_sequence_get_subscript(sequence, i);
      _filterx_tuple_append(self, filterx_object_copy(elt));
      filterx_object_unref(elt);
    }
