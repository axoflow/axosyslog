...
static struct testcase_tuple test_success_params[] =
{
  {"a\nb\nc\n",  6,  1},
  {"ab\nb\nc\n",  7,  2},
  {"abc\nb\nc\n",  8,  3},
...
StaticParameterizedTest(struct testcase_tuple *tup, test_success_params, findeom, test_success)
{
  const guchar *eom;
...
static struct testcase_tuple test_failed_params[] =
{
  {"a",  1, -1},
  {"ab",  2, -1},
  {"abc",  3, -1},
...
StaticParameterizedTest(struct testcase_tuple *tup, test_failed_params, findeom, test_failed)
{
  const guchar *eom;
  const guchar *msg = (const guchar *) tup->msg;
