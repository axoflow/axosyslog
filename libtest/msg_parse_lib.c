...
{
  cr_assert(not(log_msg_is_tag_by_name(log_message, tag_name)), "Expected message not to have '%s' tag", tag_name);
}
...
assert_log_message_value_unset(LogMessage *self, NVHandle handle)
{
  ...
  cr_assert(zero(ptr, value), "Expected value for key %s to be unset but the actual value is %s", key_name, value);
}
                                  ...
                                  const gchar *expected_value, LogMessageValueType expected_type)
{
  ...
  if (expected_value)
    {
      cr_assert(eq(str, actual_value, expected_value), "Invalid value for key %s; actual: %s, expected: %s", key_name,
                actual_value, expected_value);
    }
  else
    cr_assert(eq(str, actual_value, ""), "No value is expected for key %s but its value is %s", key_name, actual_value);

  if (expected_type != LM_VT_NONE)
    cr_assert(eq(u8, actual_type, expected_type),
                                        ...
                                        const gchar *expected_value, LogMessageValueType expected_type)
{
  ...
  if (expected_value)
    {
      cr_assert(eq(str, actual_value, expected_value), "Invalid value for $%d; actual: %s, expected: %s", index_,
                actual_value, expected_value);
    }
  else
    cr_assert(eq(str, actual_value, ""), "No value is expected for $%d but its value is %s", index_, actual_value);
...
assert_log_messages_saddr(LogMessage *log_message_a, LogMessage *log_message_b)
{
  ...
  cr_assert(eq(str, address_a, address_b), "Non-matching socket address; a: %s, b: %s", address_a, address_b);
}
...
assert_structured_data_of_messages(LogMessage *log_message_a, LogMessage *log_message_b)
{
  ...
  cr_assert(eq(str, structured_data_string_a->str, structured_data_string_b->str),
...
assert_log_message_values_equal(LogMessage *log_message_a, LogMessage *log_message_b, NVHandle handle)
{
  ...
  cr_assert(eq(str, value_a, value_b), "Non-matching value for key %s; a: %s, b: %s", key_name, value_a, value_b);
}

void
assert_log_messages_equal(LogMessage *log_message_a, LogMessage *log_message_b)
{
  cr_assert(eq(i64, log_message_a->timestamps[LM_TS_STAMP].ut_sec, log_message_b->timestamps[LM_TS_STAMP].ut_sec),
            "Timestamps are not the same");
  cr_assert(eq(u32, log_message_a->timestamps[LM_TS_STAMP].ut_usec, log_message_b->timestamps[LM_TS_STAMP].ut_usec),
            "Timestamps usec are not the same");
  cr_assert(eq(i32, log_message_a->timestamps[LM_TS_STAMP].ut_gmtoff,
               log_message_b->timestamps[LM_TS_STAMP].ut_gmtoff),
            "Timestamp offset are not the same");

  cr_assert(eq(u16, log_message_a->pri, log_message_b->pri), "Priorities are not the same");
